...
class RegisteredEmail {
  ...
  async isEmailRegistered(email) {
    const sql =
      'SELECT COUNT(*) AS count FROM registered_student_emails WHERE reg_email = ?';
  ...
  async registerEmail(email) {
    const sql =
      'INSERT INTO registered_student_emails (reg_email) VALUES (?)';
  ...
  async bulkRegisterEmails(emails = []) {
    ...
    if (values.length) {
      const sql = `
        INSERT INTO registered_student_emails (reg_email)
        VALUES ?
      `;
  ...
  async getAllRegisteredEmails() {
    const sql = 'SELECT reg_email FROM registered_student_emails';
