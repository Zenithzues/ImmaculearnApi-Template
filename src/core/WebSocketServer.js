...
export class WebSocketServer {
  ...
  constructor() {
    ...
    try {
      const persistDir = path.join(path.dirname(fileURLToPath(import.meta.url)), '../../.ydoc-persistence');
      ...
      this.logger.error('Yjs persistence init failed, falling back to memory:', err);
  ...
  async initialize(server) {
    ...
    this.wss.on('connection', async (ws, req, user, clientId) => {
      try {
        ...
        ws.on('error', (err) => this.logger.error(`WebSocket error for client ${clientId}:`, err));

        this.logger.info(`Client ${clientId} (user ${user.id}) connected to room ${roomId}`);
      } catch (err) {
  ...
  async joinRoom(ws, clientId, roomId, userId) {
    ...
    // Notify others
    await this.broadcastToRoom(roomId, { type: 'user-joined', userId, clientId }, [clientId]);
  ...
  async leaveRoom(clientId, roomId) {
    ...
    if (this.clientRooms.has(clientId)) this.clientRooms.get(clientId).delete(roomId);

    const client = this.clients.get(clientId);
    if (client) {
      await this.broadcastToRoom(roomId, { type: 'user-left', clientId, userId: client.userId }, clientId);
    }
  }
