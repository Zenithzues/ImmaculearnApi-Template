...
export class HybridDatabase {
  ...
  async initialize() {
    try {
      ...
      try {
        ...
        this.logger.warn('Supabase connection failed (continuing without it):', error.message);
  ...
  async initializeSupabaseSchema() {
    ...
    const supabaseClient = this.supabase.getAdminClient() || this.supabase.getClient();
    ...
    for (const tableSQL of tables) {
        ...
        // Execute the SQL
        const { error } = await supabaseClient.rpc('exec_sql', { sql: tableSQL });
        ...
        await supabaseClient.rpc('exec_sql', { sql: disableRLSSQL }).catch(() => {});
  ...
  async getMySQLStats() {
    try {
      const [userCount] = await this.mysql.query('SELECT COUNT(*) as count FROM users');
      const [sessionCount] = await this.mysql.query('SELECT COUNT(*) as count FROM user_sessions WHERE revoked_at IS NULL');
      
      return {
        users: userCount[0].count,
        active_sessions: sessionCount[0].count,
        connection_status: this.mysql.isConnected ? 'connected' : 'disconnected'
      };
    } catch (error) {
  ...
  async getSupabaseStats() {
    try {
      ...
      return {
        ...
        connection_status: this.supabase.isConnected ? 'connected' : 'disconnected'
      };
    } catch (error) {
      return { error: error.message };
    }
  ...
  async syncUserToSupabase(userId, role) {
    try {
        ...
        this.logger.info(`Successfully synced user ${userId} to Supabase (UUID: ${supabaseUserId})`);
